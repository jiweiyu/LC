...
//    }
    ...
    public void add(int value) {
        if(count.containsKey(value) && count.get(value)==1){
            q.remove(value);
            count.put(value,count.get(value)+1);
        }else if(!count.containsKey(value)){
            count.put(value,1);
            q.add(value);
        }else{
            count.put(value,count.get(value)+1);
...
//    }
    ...
    public int showFirstUnique__() {
        while(!queue.isEmpty()){
            if(dp.get(queue.peek()) < 2){
                return queue.peek();
            }
            queue.poll();
        }
    ...
    public void add__(int value) {
        queue.add(value);
        dp.put(value, dp.getOrDefault(value,0)+1);
    }
